...
parser.add_argument('--network', type=str, default='EnvNet3',
...
def main():
    model = WaveLSTM(in_dim=40, hidden_dim=2048, n_layer=2, n_class=50)
    # model =EnvNet3()
    if args.cuda:
        model.cuda()
    ...
    trainDataset = WaveformDataset('../data_wave/ESC-50_train.cPickle', num_slices=args.num_slices, transform=ToTensor())
    testDataset = WaveformDataset('../data_wave/ESC-50_test.cPickle', num_slices=args.num_slices, transform=ToTensor())
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.model_save_interval == 0:
            model_name = '../model/'+args.network+'_v1_epoch'+str(epoch)+'.pkl'
