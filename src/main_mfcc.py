...
parser.add_argument('--network', type=str, default='M9_mfcc',
...
#{{{
    ...
    for item in sampleSet:
        ...
        for j in range(0, len(record_data) - win_size + 1, stride):
            ...
            melspec = librosa.feature.melspectrogram(win_data, 44100, n_fft=2048, hop_length=150, n_mels=32)  # (40, 442)
...
def main_on_fold(foldNum, trainPkl, validPkl):
    ...
    exp_lr_scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[30, 60, 90], gamma=0.1)

    trainDataset = MFCCDataset(trainPkl, window_size=66150, train_slices=args.train_slices, transform=ToTensor())
    ...
    # for epoch in range(1, 2):
        ...
        if epoch % 30 == 0:
            test_acc = test_p2(model, validPkl)
    ...
    # model.changePhase()
    # print('Phase 2')
    # for param in model.conv1_2.parameters():
    #     param.requiers_grad = False
    # for param in model.bn1_2.parameters():
    #     param.requiers_grad = False
    # for param in model.conv2_2.parameters():
    #     param.requiers_grad = False
    # for param in model.bn2_2.parameters():
    #     param.requiers_grad = False
    #
    # trainDataset = MFCCDataset(trainPkl, window_size=66150, train_slices=args.train_slices, transform=ToTensor())
    #
    ...
    #         if test_acc > best_acc:
    #             best_acc = test_acc
    #             model_name = '../model/' + args.network + '_fold' + str(foldNum) + '_p2_v3_epoch' + str(epoch) + '.pkl'
    #             torch.save(model, model_name)
    #             print('model has been saved as: ' + model_name)
