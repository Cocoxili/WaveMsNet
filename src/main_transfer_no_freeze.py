...
parser.add_argument('--network', type=str, default='M9_fixed_logmel',
...
def main_on_fold(foldNum, trainPkl, validPkl):
    ...
    #     if epoch % 20 == 0:
    #         test_acc = test_p1(model, validPkl)
    #         if test_acc > best_acc:
    #             best_acc = test_acc
    #             best_model_wts = model.state_dict()
    #
    #             model_name = '../model/' + args.network + '_fold' + str(foldNum) + '_p1_v3_3.pkl'
    #             torch.save(model, model_name)
    #             print('model has been saved as: ' + model_name)


    # --------phase 2:----------
    model_name = '../model/' + args.network + '_fold' + str(foldNum) + '_p1_v3_more.pkl'
    ...
    # for epoch in range(1, 2):
        ...
        if epoch % 30 == 0:
            ...
            if test_acc > best_acc:
                best_acc = test_acc
                model_name = '../model/' + args.network + '_fold' + str(foldNum) + '_p2_v3_more_no_freeze.pkl'
                torch.save(model, model_name)
                print('model has been saved as: ' + model_name)
